...
export class WarehouseManagementService {
  ...
  async context(user: AuthUser, type: ContextType, id: string) {
    ...
    const [detail] = await this.db.query(type === 'warehouse'
      ...
      : type === 'zone'
        ...
        : `SELECT l.id,l.code,l.name,l.status,z.id "zoneId",z.code "zoneCode",z.name "zoneName",w.id "warehouseId",w.warehouse_code "warehouseCode",COALESCE(w.display_name,w.name) "warehouseName",w.warehouse_type "warehouseType" FROM warehouse_locations l JOIN warehouse_zones z ON z.id=l.zone_id JOIN warehouses w ON w.id=l.warehouse_id WHERE l.id=$1`, [id]);
  ...
  async search(user: AuthUser, keyword: string) {
    ...
    return this.db.query(`SELECT * FROM (
      SELECT 'warehouse' type,w.id,w.id "warehouseId",NULL::uuid "zoneId",NULL::uuid "locationId",NULL::uuid "itemId",w.warehouse_code code,COALESCE(w.display_name,w.name) name,w.warehouse_code "warehouseCode",NULL::text "zoneCode",NULL::text "locationCode",NULL::text unit,NULL::text quantity
      ...
      UNION ALL SELECT 'zone',z.id,w.id,z.id,NULL,NULL,z.code,z.name,w.warehouse_code,z.code,NULL,NULL,NULL FROM warehouse_zones z JOIN warehouses w ON w.id=z.warehouse_id WHERE z.deleted_at IS NULL AND ($2::uuid[] IS NULL OR w.id=ANY($2::uuid[])) AND (z.code ILIKE '%'||$1||'%' OR z.name ILIKE '%'||$1||'%')
      UNION ALL SELECT 'location',l.id,w.id,z.id,l.id,NULL,l.code,l.name,w.warehouse_code,z.code,l.code,NULL,NULL FROM warehouse_locations l JOIN warehouse_zones z ON z.id=l.zone_id JOIN warehouses w ON w.id=l.warehouse_id WHERE l.is_archived=false AND ($2::uuid[] IS NULL OR w.id=ANY($2::uuid[])) AND (l.code ILIKE '%'||$1||'%' OR l.name ILIKE '%'||$1||'%')
      UNION ALL SELECT 'material',i.id,w.id,z.id,l.id,i.id,i.item_code,i.name,w.warehouse_code,z.code,l.code,i.unit,sum(sb.on_hand_qty)::numeric(18,0)::text FROM stock_balances sb JOIN items i ON i.id=sb.item_id JOIN warehouse_locations l ON l.id=sb.location_id JOIN warehouse_zones z ON z.id=l.zone_id JOIN warehouses w ON w.id=sb.warehouse_id WHERE sb.on_hand_qty>0 AND ($2::uuid[] IS NULL OR w.id=ANY($2::uuid[])) AND (i.item_code ILIKE '%'||$1||'%' OR i.name ILIKE '%'||$1||'%' OR COALESCE(i.model,'') ILIKE '%'||$1||'%') GROUP BY i.id,w.id,z.id,l.id,w.warehouse_code,z.code,l.code
  ...
  async activity(user: AuthUser, q: any) {
    ...
    const rows = await this.db.query(`SELECT t.id,t.source_document_id "documentId",d.document_no "documentNo",d.document_type "documentType",t.created_at "createdAt",t.delta_qty::text "deltaQty",i.id "itemId",i.item_code "itemCode",i.name "itemName",i.unit,l.id "locationId",l.code "locationCode",z.code "zoneCode",w.warehouse_code "warehouseCode",COALESCE(t.operator_name,u.employee_name,u.name,u.username,'—') operator
    ...
    for (const group of moves.values()) { const out = group.find(row => Number(row.deltaQty) < 0), inbound = group.find(row => Number(row.deltaQty) > 0); result.push({ ...out, deltaQty: out ? String(Math.abs(Number(out.deltaQty))) : inbound?.deltaQty, direction: 'MOVE', sourcePath: out ? `${out.warehouseCode} / ${out.zoneCode} / ${out.locationCode}` : null, targetPath: inbound ? `${inbound.warehouseCode} / ${inbound.zoneCode} / ${inbound.locationCode}` : null }); }
  ...
  async assertOperationAllowed(qr: QueryRunner, locations: Array<string | null | undefined>, itemRules: Array<{ locationId?: string; itemId?: string }> = []) {
    ...
    if (locked.length) throw new BusinessException('WAREHOUSE_OPERATION_LOCKED', `${locked[0].code} 当前禁止库存作业：${locked[0].reason}`, HttpStatus.CONFLICT);
