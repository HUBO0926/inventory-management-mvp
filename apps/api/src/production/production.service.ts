...
export class ProductionService{
  constructor(private readonly db:DataSource,private readonly posting:InventoryPostingService,private readonly audit:AuditService,private readonly approvalHistory:ApprovalHistoryService,private readonly picking:PickingOrdersService,private readonly reservations:StockReservationService,private readonly numbers:BusinessNumberService,private readonly workflow:ApprovalWorkflowService){}
  async create(dto:any,userId:string){const qr=this.db.createQueryRunner();await qr.connect();await qr.startTransaction();try{const [output]=await qr.query(`SELECT id FROM items WHERE id=$1 AND item_type='FINISHED_GOOD' AND status='ACTIVE' AND deleted_at IS NULL`,[dto.finishedGoodId]);if(!output)throw new BusinessException('VALIDATION_ERROR','产出成品必须是启用的成品档案');if(dto.defaultIssueWarehouseId){const [warehouse]=await qr.query(`SELECT id FROM warehouses WHERE id=$1 AND warehouse_type='RAW' AND status='ACTIVE' AND deleted_at IS NULL`,[dto.defaultIssueWarehouseId]);if(!warehouse)throw new BusinessException('VALIDATION_ERROR','默认领料仓库必须是启用的原材料仓库');}const [bom]=await qr.query(`SELECT id FROM boms WHERE finished_good_id=$1 AND status='ACTIVE' AND deleted_at IS NULL`,[dto.finishedGoodId]);if(!bom)throw new BusinessException('VALIDATION_ERROR','产出成品没有启用 BOM');const lines=await qr.query(`SELECT material_id,qty_per FROM bom_items WHERE bom_id=$1 ORDER BY material_id`,[bom.id]);if(!lines.length)throw new BusinessException('VALIDATION_ERROR','BOM 没有组成物料明细');const id=randomUUID(),orderNo=await this.insertProductionOrderHeader(qr,id,dto,userId);for(const l of lines){const required=new Decimal(l.qty_per).mul(dto.plannedQty).toFixed(0);await qr.query(`INSERT INTO production_order_materials(id,production_order_id,material_id,qty_per,required_qty) VALUES($1,$2,$3,$4,$5)`,[randomUUID(),id,l.material_id,l.qty_per,required]);}await this.audit.log(userId,'CREATE_PRODUCTION_ORDER','production_orders',id,{orderNo},qr.manager);await qr.commitTransaction();return this.get(id);}catch(e){await qr.rollbackTransaction();throw e;}finally{await qr.release();}}
    ...
    const rows=await this.db.query(`
      WITH reserved AS (
        ...
        SELECT pm.production_order_id,
          ...
          COALESCE(sum(GREATEST(pm.required_qty-(pm.issued_qty-pm.returned_qty),0)),0)::numeric(18,0) "pendingIssueQty",
          COALESCE(sum(GREATEST(pm.spare_issued_qty-pm.spare_returned_qty,0)),0)::numeric(18,0) "pendingReturnQty"
