...
export class PickingOrdersService {
  ...
  async shortages(orderId: string, manager: DataSource | QueryRunner = this.db) {
    const rows = await manager.query(
      `WITH reserved AS (
       ...
       SELECT m.material_id "materialId",i.item_code "itemCode",i.name "itemName",i.model,i.spec,i.unit,
         ...
         (m.issued_qty-m.returned_qty)::numeric(18,4) "netIssuedQty",
         GREATEST(m.required_qty-(m.issued_qty-m.returned_qty),0)::numeric(18,4) "pendingQty",
         GREATEST(COALESCE(inv.qty,0)-COALESCE(r.qty,0),0)::numeric(18,4) "availableQty",
         GREATEST(GREATEST(m.required_qty-(m.issued_qty-m.returned_qty),0)-GREATEST(COALESCE(inv.qty,0)-COALESCE(r.qty,0),0),0)::numeric(18,4) "shortageQty",
    ...
    return rows.map((row: any) => Object.fromEntries(Object.entries(row).map(([key,value]) => [
      key,
      key.endsWith('Qty') ? new Decimal(value as any || 0).toFixed(4) : value,
    ])));
  }
  ...
  async createManual(orderId: string, dto: any, key: string | undefined, user: AuthUser) {
    return this.posting.executeIdempotent(user.id,key,'POST:/production-orders/:id/picking-orders/manual',{orderId,...dto},async qr => {
      ...
      const lines = normalized.flatMap((material: any) => material.allocations.map((allocation: any) => ({
        ...
        quantity:new Decimal(allocation.normalQty || 0).add(allocation.spareQty || 0).toFixed(4),
        normalQty:new Decimal(allocation.normalQty || 0).toFixed(4),
        spareQty:new Decimal(allocation.spareQty || 0).toFixed(4),
  ...
  async createAuto(orderId: string, dto: any, key: string | undefined, user: AuthUser) {
    return this.posting.executeIdempotent(user.id,key,'POST:/production-orders/:id/picking-orders/auto-generate',{orderId,...dto},async qr => {
      ...
      const lines = materials.flatMap((material: any) => material.allocations.map((allocation: any) => ({
        itemId:material.materialId,quantity:new Decimal(allocation.normalQty).add(allocation.spareQty).toFixed(4),
  ...
  availableLocations(q:any) {
    ...
    params.push(requested.toFixed(4));
    return this.db.query(`SELECT w.id "warehouseId",w.warehouse_code "warehouseCode",z.id "zoneId",z.code "zoneCode",l.id "locationId",l.code "locationCode",b.id "batchId",b.batch_no "batchNo",
      GREATEST(sb.on_hand_qty-COALESCE(sb.frozen_qty,0)-COALESCE(r.qty,0),0)::numeric(18,4)::text "availableQty"
  ...
  private validateMaterials(materials:any[],shortages:any[]) {
    ...
    return materials.map(material=>{
      ...
      this.validateSpare(current,spare.toFixed(4));
      ...
      return{...material,normalQty:normal.toFixed(4),spareQty:spare.toFixed(4),allocations};
    });
  }

  private validateSpare(item:any,value:string){const spare=new Decimal(value||0);if(spare.isNegative()||spare.gt(new Decimal(item.requiredQty).mul('0.1')))throw new BusinessException('VALIDATION_ERROR',`${item.itemCode} 备用件不能超过 BOM 需求量的 10%`);}

  private async allocate(db:any,itemId:string,normalQty:string,spareQty:string,preferred:string) {
    const rows=await db.query(`SELECT sb.warehouse_id "warehouseId",w.warehouse_code "warehouseCode",sb.location_id "locationId",l.code "locationCode",sb.batch_id "batchId",
      GREATEST(sb.on_hand_qty-COALESCE(sb.frozen_qty,0)-COALESCE(r.qty,0),0)::numeric(18,4) available
      FROM stock_balances sb JOIN warehouses w ON w.id=sb.warehouse_id JOIN warehouse_locations l ON l.id=sb.location_id
      LEFT JOIN (SELECT warehouse_id,location_id,item_id,batch_id,sum(quantity) qty FROM stock_reservations WHERE status='ACTIVE' GROUP BY warehouse_id,location_id,item_id,batch_id) r
       ON r.warehouse_id=sb.warehouse_id AND r.location_id=sb.location_id AND r.item_id=sb.item_id AND r.batch_id IS NOT DISTINCT FROM sb.batch_id
