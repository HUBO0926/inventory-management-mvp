...
export type WarehouseContext =
  ...
  | { type:'zone';warehouseId:string;zoneId:string }
...
const can=(user:User|undefined,permission:string)=>user?.role==='ADMIN'||Boolean(user?.permissions?.includes(permission))||Boolean(user?.isWarehouseManager&&managerPermissions.has(permission));
const contextId=(context?:WarehouseContext)=>context?.type==='warehouse'?context.warehouseId:context?.type==='zone'?context.zoneId:context?.type==='location'||context?.type==='material'?context.locationId:undefined;
const ContextState=({value}:{value:string})=><Tag color={stateColor[value]||'default'}><i className={`warehouse-state-dot ${value.toLowerCase()}`} />{stateText[value]||value}</Tag>;
...
function OperationModal({open,context,detail,seed,user,onClose,onDone,loadWarehouseLocations}:{[key:string]:any}){
  ...
  if(!open)return null;
  const targetWarehouse=async(id:string)=>{form.setFieldValue('targetLocationId',undefined);const rows=await loadWarehouseLocations(id);setTargetLocations(rows);};
  const previewAllocation=async()=>{try{const values=await form.validateFields();const mode=operation==='inbound'?'INBOUND':operation==='move'?'MOVE':'OUTBOUND';const result=await api('/stock-documents/allocation-preview',{method:'POST',body:JSON.stringify({operationType:mode,warehouseId,zoneId:context?.type==='zone'?context.zoneId:undefined,locationId:values.locationId,targetWarehouseId:values.targetWarehouseId,targetLocationId:values.targetLocationId,lines:values.lines.map((line:any)=>({itemId:line.itemId,quantity:String(line.quantity)}))})});setPreview({...result,allocations:(result.allocations||[]).map((row:any,index:number)=>({...row,clientKey:`${Date.now()}-${index}`}))});message.success('已生成可编辑分配预览');}catch(e:any){message.error(e.message);}};
  const editPreview=(clientKey:string,quantity:number|null)=>setPreview((current:any)=>({...current,allocations:current.allocations.map((row:any)=>row.clientKey===clientKey?{...row,quantity:String(quantity||0)}:row)}));
  const save=async(submit:boolean)=>{setSaving(true);let created:any;try{const values=await form.validateFields();if(operation==='count'){created=await api('/stock-documents/stock-check',{method:'POST',body:JSON.stringify({warehouseId,notes:values.notes,lines:values.lines.map((line:any)=>({locationId:values.locationId||context?.locationId,itemId:line.itemId,batchId:line.batchId,countedQty:String(line.countedQty),notes:line.notes}))})});}
      else{if(!preview&&!values.locationId)throw new Error('请选择具体库位，或先生成自动分配预览');if(operation==='move'&&!preview&&!values.targetLocationId)throw new Error('请选择目标库位，或先生成自动分配预览');let lines:any[]=preview?.allocations.map(({clientKey,...line}:any)=>line)||values.lines.map((line:any)=>({...line,quantity:String(line.quantity),locationId:values.locationId||context?.locationId,targetWarehouseId:values.targetWarehouseId,targetLocationId:values.targetLocationId}));const endpoint=operation==='inbound'?(warehouseType==='RAW'?'/stock-documents/material-inbound':'/stock-documents/finished-inbound'):operation==='outbound'?'/stock-documents/finished-outbound':operation==='move'?'/stock-documents/move':'/stock-documents/inventory-adjustment';if(operation==='adjust')lines=lines.map((line:any)=>({...line,locationId:values.locationId||context?.locationId,adjustmentQty:String(line.adjustmentQty)}));created=await api(endpoint,{method:'POST',body:JSON.stringify({warehouseId,notes:values.notes,lines})});}
      if(submit){try{await api(`/stock-documents/${created.id}/submit`,{method:'POST'});message.success('单据已创建并提交审批');}catch(e:any){message.warning(`草稿 ${created.documentNo||created.id} 已保留，提交失败：${e.message}`);onDone();return;}}else message.success('草稿已保存');onDone();
...
export function WarehouseManagementPage({user}:{user?:User}){
  ...
  const updateUrl=(context:WarehouseContext,node:any)=>{const p=new URLSearchParams();p.set('warehouse',node.warehouseCode||warehouses.find(w=>w.id===context.warehouseId)?.warehouseCode||'');if(context.type!=='warehouse')p.set('zone',node.zoneCode||node.code);if(context.type==='location'||context.type==='material')p.set('location',node.locationCode||node.code);nav(`/inventory/warehouse-management?${p}`,{replace:true});};
  ...
  const onMore=({key}:{key:string})=>{if(key==='adjust')start('adjust');if(key==='capacity'){setCapacity({});capacityForm.resetFields();}if(key==='flow')nav(`/inventory/management?tab=flows&warehouseId=${selected?.warehouseId}`);if(key==='document')nav(`/inventory/management?tab=documents&warehouseId=${selected?.warehouseId}`);if(key==='edit')nav('/warehouse-archive');if(key==='lock'){if(detail.lock)void api(`/warehouse-management/operation-locks/${detail.lock.id}/release`,{method:'POST'}).then(()=>{message.success('作业锁已解除');load();}).catch((e:any)=>message.error(e.message));else Modal.confirm({title:'冻结当前仓储空间',content:<Input id="lock-reason" placeholder="请输入冻结原因"/>,onOk:()=>api('/warehouse-management/operation-locks',{method:'POST',body:JSON.stringify({scopeType:detail.type,id:detail.id,reason:(document.getElementById('lock-reason') as HTMLInputElement)?.value||'人工冻结'})}).then(()=>{message.success('已冻结');load();})});}};
  const saveCapacity=async(v:any)=>{const locationId=selected&&'locationId'in selected?selected.locationId:undefined;try{await api('/location-item-capacities',{method:'PUT',body:JSON.stringify({locationId,itemId:capacity.itemId,capacity:String(v.capacity),notes:v.notes})});message.success('容量已保存');setCapacity(undefined);await pick(selected!,selectedNode,false);}catch(e:any){message.error(e.message);}};
  const rule=async(row:any)=>{const locationId=selected&&'locationId'in selected?selected.locationId:'';try{if(row.allowed===false)await api(`/warehouse-management/locations/${locationId}/item-rules/${row.itemId}`,{method:'DELETE'});else await api(`/warehouse-management/locations/${locationId}/item-rules/${row.itemId}`,{method:'PUT',body:JSON.stringify({allowed:false,notes:'仓库管理工作台设置'})});message.success(row.allowed===false?'已恢复默认准入':'已禁止该物料存入此库位');await pick(selected!,selectedNode,false);}catch(e:any){message.error(e.message);}};
  const childRows=detail?.type==='warehouse'?(zonesByWarehouse[detail.id]||[]):detail?.type==='zone'?(locationsByZone[detail.id]||[]):[];
