...
export function UsersV110Page() {
  ...
  return <PageScaffold title="账号管理" subtitle="账号绑定动态角色；停用账号后，已签发 JWT 在下一次请求时立即失效。">
    ...
    <Modal title="新增账号" open={open} onCancel={() => setOpen(false)} onOk={() => form.submit()}><Form form={form} layout="vertical" onFinish={save}><Form.Item label="账号" name="username" rules={[{ required: true }]}><Input /></Form.Item><Form.Item label="姓名" name="name" rules={[{ required: true }]}><Input /></Form.Item><Form.Item label="角色" name="roleId" rules={[{ required: true }]}><Select options={roles.filter(role => role.status === 'ACTIVE').map(role => ({ value: role.id, label: `${role.name} (${role.code})` }))} /></Form.Item><Form.Item label="初始密码" name="password" rules={[{ required: true, min: 8 }]}><Input.Password /></Form.Item></Form></Modal>
  </PageScaffold>;
}
