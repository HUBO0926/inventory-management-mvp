  ...
  return <PageScaffold title="库位容量" subtitle="按仓库、库区、库位配置物料容量；未配置容量即不限量。" extra={<Space><Button icon={<ReloadOutlined />} loading={loading} onClick={() => void load()}>刷新</Button>{manageable && <Button type="primary" icon={<PlusOutlined />} onClick={() => start()}>新增容量</Button>}</Space>}>
    <div className="capacity-workbench">
      <Card className="capacity-tree" title="仓储层级" loading={loading}>
        {!warehouses.length ? <Empty description="暂无可查看仓库" /> : warehouses.map((warehouse: any) => <section key={warehouse.id} className="capacity-tree-warehouse"><strong>{warehouse.name} <small>{warehouse.warehouseCode}</small></strong>{warehouse.zones.map((zone: any) => <div key={zone.id} className="capacity-tree-zone"><span><ApartmentOutlined /> {zone.name} <small>{zone.code} · {zone.locationCount} 库位</small></span>{zone.locations.map((row: any) => <button type="button" key={row.locationId} onClick={() => setSelected(row)} className={selected?.locationId === row.locationId ? 'is-selected' : ''}><i className={`capacity-dot ${String(row.status).toLowerCase()}`} /><LocationName location={row}/><small>{row.itemTypeCount ? `${row.itemTypeCount} 物料` : '未配置容量'} · {labels[row.status] || row.status}</small></button>)}</div>)}</section>)}</Card>
      <section className="capacity-detail">
        {selected ? <><Card className="capacity-overview" title={<LocationName location={selected}/>} extra={<Button icon={<DatabaseOutlined />} onClick={() => nav(`/virtual-warehouse?warehouse=${selected.warehouseCode}&zone=${selected.zoneCode}&location=${selected.locationCode}`)}>在虚拟仓库查看</Button>}><div className="capacity-overview-grid"><span><small>仓库</small>{selected.warehouseName} ({selected.warehouseCode})</span><span><small>库区</small>{selected.zoneName} ({selected.zoneCode})</span><span><small>库位状态</small><Tag color={colors[selected.status]}>{labels[selected.status]}</Tag></span><span><small>物料种类</small>{selected.itemTypeCount}</span></div></Card>
    ...
    <Modal title={editing ? '编辑库位物料容量' : '新增库位物料容量'} open={open} onCancel={() => setOpen(false)} onOk={() => form.submit()} destroyOnHidden><Form form={form} layout="vertical" onFinish={save}>
      ...
      <Form.Item name="locationId" label="库位" rules={[{ required: true }]}><Select disabled={contextLocked} options={locations.map((row: any) => ({ value: row.locationId, label: locationDisplayName(row) }))} /></Form.Item>
